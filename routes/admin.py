...
def _log_task_activity(db: Session, task_id: int, actor_user_id: int | None, event_type: str, meta: dict | None = None):
    ev = TaskActivity(
        task_id=task_id,
...
def _apply_auto_assignment(db: Session, pmc_id: int, task: Task):
    # simple rules engine: first matching enabled rule wins
    rules = (
    ...
    for r in rules:
        ...
        if not exists:
            db.add(TaskAssignee(task_id=task.id, user_id=r.assign_to_user_id, assigned_at=_now_utc()))
        _log_task_activity(db, task.id, None, "assigned", {"user_id": r.assign_to_user_id, "by": "auto_rule"})
        break
...
def list_tasks(
    ...
    if property_id:
        qry = qry.filter(Task.property_id == int(property_id))
    if category:
        qry = qry.filter(func.lower(func.coalesce(Task.category, "")) == category.strip().lower())

    if assignee_id:
        qry = qry.join(TaskAssignee, TaskAssignee.task_id == Task.id).filter(TaskAssignee.user_id == int(assignee_id))

    tasks = qry.order_by(Task.updated_at.desc(), Task.id.desc()).limit(500).all()
    rows = [_task_to_row(db, t) for t in tasks]
...
def task_detail(task_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    comments = (
        db.query(TaskComment, PMCUser.full_name, PMCUser.email)
    ...
    activity = (
        db.query(TaskActivity, PMCUser.full_name, PMCUser.email)
        ...
        "activity": [
            {
                ...
                "actor": {"name": name, "email": email} if name or email else None,
            }
            for (ev, name, email) in activity
        ],
    }
...
async def batch_tasks(request: Request, db: Session = Depends(get_db)):
    ...
    payload = await request.json()
    task_ids = [int(x) for x in (payload.get("task_ids") or []) if x]
    ...
    tasks = db.query(Task).filter(Task.pmc_id == pmc_obj.id, Task.id.in_(task_ids)).all()
    if not tasks:
    ...
    if action == "delete":
        for t in tasks:
            _log_task_activity(db, t.id, actor_id, "deleted", {})
            db.delete(t)
        db.commit()
...
async def add_task_comment(task_id: int, request: Request, db: Session = Depends(get_db)):
    user_role, pmc_obj, pmc_user, *_ = get_user_role_and_scope(request, db)
    if user_role != "pmc" or not pmc_obj:
        raise HTTPException(status_code=403)
    ...
    c = TaskComment(task_id=t.id, user_id=getattr(pmc_user, "id", None), body=body, created_at=_now_utc())
    db.add(c)
    _log_task_activity(db, t.id, getattr(pmc_user, "id", None), "comment_added", {})
    db.commit()
    return {"ok": True}
...
async def upload_task_attachment(
    ...
    file_type = "video" if (mime or "").startswith("video/") or ext in {".mp4", ".mov", ".webm"} else "photo"
    ...
    db.add(a)
    _log_task_activity(db, t.id, getattr(pmc_user, "id", None), "attachment_added", {"file_url": url, "file_type": file_type})
    db.commit()
