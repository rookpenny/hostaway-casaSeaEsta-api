...
window.DashboardOverview = {
  ...
  initPortfolioChart() {
    const canvas = document.getElementById("overviewPortfolioChart");
    if (!canvas || !window.Chart) return;

    const liveEl = document.getElementById("stat-live");
    const offlineEl = document.getElementById("stat-offline");

    const live = Number(liveEl?.textContent || 0);
    const offline = Number(offlineEl?.textContent || 0);

    if (window.__overviewPortfolioChart) {
      window.__overviewPortfolioChart.destroy();
    }

    window.__overviewPortfolioChart = new Chart(canvas.getContext("2d"), {
      type: "doughnut",
      data: {
        labels: ["Live", "Offline"],
        datasets: [{
...
document.addEventListener("DOMContentLoaded", () => {
  window.DashboardOverview?.initPortfolioChart();
});
