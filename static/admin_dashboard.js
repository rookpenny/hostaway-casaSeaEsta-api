...
window.CONTENT_LOCKED = IS_LOCKED; // if you want global
  ...
  let manualQuill = null;
  let manualLoadedHTML = "";
  let manualDirty = false;

  function ensureManualQuill() {
    if (manualQuill) return;
    if (!window.Quill) return;

    manualQuill = new Quill("#manualQuillEditor", {
      theme: "snow",
      modules: {
        toolbar: [
          [{ header: [1, 2, 3, false] }],
      ...
      placeholder: "Write the house manual…",
    });

    manualQuill.on("text-change", () => {
      const html = manualQuill.root.innerHTML || "";
      manualDirty = (html !== manualLoadedHTML);
      document.getElementById("manualSaveBtn").disabled = !manualDirty;
    });
  }
  ...
  function looksLikeHTML(text) {
    const t = (text || "").trim();
    // very simple heuristic: contains tags
  ...
  async function openInlineManual(filePath, propertyName) {
    ensureManualQuill();
    if (!manualQuill) return false;

    const wrap = document.getElementById("manualPanelWrap");
    const label = document.getElementById("manualScopeLabel");
    ...
    // Load the txt
    try {
      // ✅ You need a raw read endpoint that returns the file content as text/plain
      // Example: GET /admin/file-raw?file=<path>
      const res = await fetch(`/admin/file-raw?file=${encodeURIComponent(filePath)}`, {
      ...
      manualQuill.root.innerHTML = (html || "<p></p>");
      manualLoadedHTML = manualQuill.root.innerHTML;
      manualDirty = false;

      document.getElementById("manualSaveBtn").disabled = true;
      document.getElementById("manualSaveBtn").onclick = saveInlineManual;
    } catch (e) {
      console.error(e);
      manualQuill.root.innerHTML = "<p></p>";
      manualLoadedHTML = manualQuill.root.innerHTML;
      manualDirty = false;
      document.getElementById("manualSaveBtn").disabled = true;
    }
  ...
  async function saveInlineManual() {
    if (!manualQuill) return;
    const filePath = document.getElementById("manualFilePath")?.value;
    if (!filePath) return;

    const html = manualQuill.root.innerHTML || "";

    try {
      // ✅ You need a save endpoint that writes to the same .txt file
      // Example: POST /admin/file-save { file, content }
      const res = await fetch("/admin/file-save", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ file: filePath, content: html })
      });

      if (!res.ok) throw new Error(`Save failed: ${res.status}`);

      manualLoadedHTML = html;
      manualDirty = false;
      document.getElementById("manualSaveBtn").disabled = true;
    } catch (e) {
      console.error(e);
  ...
  function closeInlineManual() {
    document.getElementById("manualPanelWrap")?.classList.add("hidden");
    document.getElementById("propertiesGridWrap")?.classList.remove("hidden");
    document.getElementById("propertiesHeaderCard")?.classList.remove("hidden");
    document.getElementById("propertiesHeaderCard")?.scrollIntoView({ behavior: "smooth", block: "start" });
  }
